...
namespace mapReduce {
    std::tuple<int, int, std::string, int, int> Coordinator::askTask(int) {
        ...
        if(!isMapFinished){
            ...
            if(index != -1){
                ...
                return std::tuple<int, int, std::string, int, int>(static_cast<int>(mapReduce::mr_tasktype::MAP), index, files[index], 0, 0);
            }
            else{
                return std::tuple<int, int, std::string, int, int>(static_cast<int>(mapReduce::mr_tasktype::NONE), -1, "", 0, 0);
            }
        }
            ...
            if(index != -1){
                ...
                return std::tuple<int, int, std::string, int, int>(static_cast<int>(mapReduce::mr_tasktype::REDUCE), index, "", reduce_task_handout.size(), files.size());
            }
            else{
                return std::tuple<int, int, std::string, int, int>(static_cast<int>(mapReduce::mr_tasktype::NONE), -1, "", 0, 0);
            }
            ...
            if(!isMergeStart){
                isMergeStart = true;
                return std::tuple<int, int, std::string, int, int>(static_cast<int>(mapReduce::mr_tasktype::NONE), 0, "", reduce_task_handout.size(), files.size());
            }
            else{
                return std::tuple<int, int, std::string, int, int>(static_cast<int>(mapReduce::mr_tasktype::NONE), -1, "", 0, 0);
            }
    ...
    int Coordinator::submitTask(int taskType, int index) {
        ...
        if(taskType == mapReduce::mr_tasktype::MAP){
            ...
            isReduceFinished = reduce_finished;
        }
        else{
            isFinished = true;
        }
